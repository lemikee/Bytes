...
class Slider extends React.Component {
  ...
  unselectBox(index) {
    ...
    for (let i = 0; i < boxes.length; i++) {
      if (element.classList.contains("showcase-photo-select")) selected = true;
    }
  ...
  render() {
    if (!this.props.business.picLinks) {
      return <div></div>;
    }
    ...
    return (
      <div className="slider">
        <div className="showcase-photo-boxes">
          <div
            className="showcase-photo-box showcase-photo-1"
            onMouseEnter={this.mouseEnter(1)}
            onMouseLeave={this.mouseLeave(1)}
          >
            <img
              src={this.props.business.picLinks[this.state.currentIndex]}
            ></img>
          </div>
          <div
            className="showcase-photo-box showcase-photo-2 showcase-photo-select"
            onMouseEnter={this.mouseEnter(2)}
            onMouseLeave={this.mouseLeave(2)}
          >
            <img
              src={this.props.business.picLinks[this.state.currentIndex + 1]}
            ></img>
          </div>
          <div
            className="showcase-photo-box showcase-photo-3"
            onMouseEnter={this.mouseEnter(3)}
            onMouseLeave={this.mouseLeave(3)}
          >
            <img
              src={this.props.business.picLinks[this.state.currentIndex + 2]}
            ></img>
          </div>
          <div className="slider-loader">
            <img
              src={this.props.business.picLinks[this.state.currentIndex + 3]}
            ></img>
          </div>
        </div>
